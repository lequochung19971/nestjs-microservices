...
import { Search, Plus, Folder, ArrowUpDown, ArrowUp, ArrowDown } from 'lucide-react';
...
import { flexRender, type HeaderGroup, type Row, type Cell } from '@tanstack/react-table';
...
export function CategoriesTable({ onCreateCategory, onEditCategory }: CategoriesTableProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-md border">
        <Table>
          <TableHeader>
            {table.getHeaderGroups().map((headerGroup: HeaderGroup<CategoryDto>) => (
            ...
            {categories.length === 0 ? (
              ...
              categories.map((row: Row<CategoryDto>) => (
                <TableRow key={row.id} data-state={row.getIsSelected() && 'selected'}>
                  {row.getVisibleCells().map((cell: Cell<CategoryDto, unknown>) => (
